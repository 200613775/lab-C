...
const createScene = async function() {
    ...
    scene.onPointerDown = async () => {
        if (lastHitTest && marker.isVisible) {
            ...
            // changed to mesh for lab
            const box = buildRandomMesh();
    ...
    const choice = meshTypes[Math.floor(Math.random() * meshTypes.length)];
    ...
    switch(choice) {
        ...
        case "box":
            mesh = BABYLON.MeshBuilder.CreateBox("mesh", { size: size }, scene);
            break;

        case "sphere":
            mesh = BABYLON.MeshBuilder.CreateSphere("mesh", { diameter: size }, scene);
            break;

        case "cylinder":
            mesh = BABYLON.MeshBuilder.CreateCylinder("mesh", { height: size, diameter: size }, scene);
            break;

        case "torus":
            mesh = BABYLON.MeshBuilder.CreateTorus("mesh", { diameter: size, thickness: size / 3 }, scene);
            break;
    }
    ...
    const mat = new BABYLON.StandardMaterial("meshMat", scene);
